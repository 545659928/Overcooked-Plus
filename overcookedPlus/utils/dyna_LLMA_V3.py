    ...
    def _checkMacroActionDone(self):
        ...
        for idx, agent in enumerate(self.agent):
            if not self.macroAgent[idx].cur_macro_action_done:
                ...
                if (
                    self.macroActionName[macro_action]
                    in ["go to knife 1", "go to knife 2"]
                    ...
                    if self._calDistance(agent.x, agent.y, target_x, target_y) == 1:
                        self.macroAgent[idx].cur_macro_action_done = True
                elif self.macroActionName[macro_action] in [
                    "get tomato",
                    "get lettuce",
                    "get onion",
                ]:
                    ...
                    self.macroActionName[macro_action] == "deliver"
                    and not agent.holding
                ):
                    target_x, target_y = self._findPOitem(agent, macro_action)
                    if self._calDistance(agent.x, agent.y, target_x, target_y) == 1:
                    ...
                    self.mapType in ["B", "C"]
                    and self.macroActionName[macro_action] == "go to counter "
                    and not agent.holding
                ):
                    target_x = 0
                ...
                if self.macroActionName[macro_action] in [
                    "get tomato",
                    "get lettuce",
                    "get onion",
                    ...
                    target_x, target_y = self._findPOitem(agent, macro_action)
                    macroAction2Item = {
                        "get tomato": self.tomato[0],
                        "get lettuce": self.lettuce[0],
                        "get onion": self.onion[0],
                        "get plate 1": self.plate[0],
                    }
    ...
    def _computeLowLevelActions(self, macro_actions):
        ...
        for idx, agent in enumerate(self.agent):
            ...
            if self.macroActionName[macro_action] == "chop":
                ...
                and agent.x == 1
                and agent.y == 1
                and ITEMNAME[agent.pomap[2][1]] == "agent"
            ):
                ...
                if self.macroActionName[macro_action] in [
                    ...
                    if (
                        target_x >= agent.x - self.obs_radius
                        and target_x <= agent.x + self.obs_radius
                        and target_y >= agent.y - self.obs_radius
                        and target_y <= agent.y + self.obs_radius
                ...
                if (
                    ...
                    primitive_action = ACTIONIDX["right"]
                elif ITEMNAME[agent.pomap[target_x][target_y]] == "agent" and (
                    (
                        target_x >= agent.x - self.obs_radius
                        and target_x <= agent.x + self.obs_radius
                        and target_y >= agent.y - self.obs_radius
                    ...
                    if self._calDistance(agent.x, agent.y, target_x, target_y) == 1:
                        self.macroAgent[idx].cur_macro_action_done = True
                        if (
                            self.macroActionName[macro_action] in ["get plate 1"]
                            and agent.holding
                        ):
    ...
    def _navigate(self, agent, target_x, target_y):
        ...
        q.put(
            AStarAgent(
                ...
                agent.y,
                0,
                self._calDistance(agent.x, agent.y, target_x, target_y),
                None,
                [],
                0,
            )
                    ...
                    if new_name == "space" or new_name == "agent":
                        ...
                        q.put(
                            AStarAgent(
                                ...
                                f,
                                init_action,
                                aStarAgent.history_action + [actionIdx[action]],
                                pass_agent,
                            )
                        )
        ...
        for idx, agent in enumerate(self.agent):
            if self.macroAgent[idx].cur_macro_action_done:
                ...
                if self.obs_radius > 0:
                    ...
                    new_image_width = int(
                        (old_image_width / self.xlen)
                        * (self.xlen + 2 * (self.obs_radius - 1))
                    )
                    new_image_height = int(
                        (old_image_height / self.ylen)
